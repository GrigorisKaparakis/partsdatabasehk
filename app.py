...
st.set_page_config(page_title="Moto ERP Multi-Sheet", layout="wide")
...
def get_sheet_data(worksheet_name):
    try:
        # Διαβάζουμε το συγκεκριμένο worksheet
...
with t_active:
    ...
    # Φόρμα Καταχώρησης (Πάντα στο Sheet1)
    with st.expander("➕ ΝΕΑ ΠΑΡΑΓΓΕΛΙΑ"):
        with st.form("new_order"):
            ...
            if st.form_submit_button("✅ ΚΑΤΑΧΩΡΗΣΗ"):
                ...
                updated = pd.concat([df_active, new_row], ignore_index=True)
                conn.update(worksheet="Sheet1", data=updated)
                st.rerun()

    # Επεξεργασία Τρεχουσών
...
with t_done:
    ...
    st.dataframe(df_done_view[df_done_view["ΕΤΑΙΡΕΙΑ"] == brand_filter], width="stretch")

# --- TAB: ΑΚΥΡΩΜΕΝΑ (Sheet3) ---
with t_cancel:
    df_cancel_view = get_sheet_data("Sheet3")
    st.dataframe(df_cancel_view[df_cancel_view["ΕΤΑΙΡΕΙΑ"] == brand_filter], width="stretch")
